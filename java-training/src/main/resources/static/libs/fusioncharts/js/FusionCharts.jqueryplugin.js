...
/*global document: false, FusionCharts: false, window: false, jQuery: false */
...
(function () {
    ...
    global = FusionCharts(['private', 'HTMLTableDataHandler']);
     ...
     *  sanitized.
     */
    var sanitizeNodesArray = function (nodeArr) {
     ...
     *
     */
    var mergeJSON = function (srcJSON, targetJSON) {
        var i, len, item;
     ...
     *
     *  @param {HTML Object} tbl Is the table whose tbody needs to fetched.
     *
     */
    var getTbody = function (tbl) {
     ...
     *
     *  @param {HTML Object} nodeEle The HTML element whose text is needed.
     */
    var getTextFromNode = function (nodeEle) {
     ...
     */
    var getColumnArr = function (rowArr) {

        var i, j, l, len, cellArr, rowSpanInc, colSpan = 1, columnArrIdx,
            rowSpan = {}, returnObj = [];
    ...
    var checkData = function (nodeArr, j, index) {

        var i, l, childArr = sanitizeNodesArray(nodeArr[j].childNodes), text;
     ...
     *
     */
    var getLabels = function (nodeArr, ignoreArr, index) {
        ...
        var isRowLabel = (nodeArr[0].nodeType !== undefined) ? true : false;
        var maxIdx, spanLength, totalSpanLength = 0;
        for (j = 0, len = childArr.length; j < len; j += 1) {
            spanLength = 0;
            if (isRowLabel) {
    ...
    var extractDataFromTable = function (tbl, opts) {

        if (typeof tbl === "string") {
            tbl = document.getElementById(tbl);
        }

        if ((typeof jQuery !== "undefined") && (tbl instanceof jQuery)) {
            tbl = tbl.get(0);
        }
        ...
        var i, j, rowCells, cellText, dataMap = {}, mapColumnIdx, columnSpan,
            len, item, rowSpan, cellEle, columnSpanObj = {}, rowSpanObj = {},
            tableRows = sanitizeNodesArray(getThead(tbl)).concat(sanitizeNodesArray(getTbody(tbl).childNodes)),
            l = tableRows.length, dataRows = 0, dataColumns = 0, tempColumn = 0;
        ...
        return {
            data: dataMap,
            chartType: ((dataRows > 1 && dataColumns > 1) ? 'multi' : 'single'),
    ...
    var createChartFromTable = function (data, obj, config) {
        ...
        var i, item1, item2, categoryArr, datasetArr, chartJSON = {},
            datasets = {}, dataObj = extractDataFromTable(data, opts),
            dataMap = dataObj.data, labelMap, legendMap;
        ...
        chartObjects.each(function () {
            ...
            if (renderFlag) {
                ...
                if (filterOpts.swfUrl) {
                    newChart.src = filterOpts.swfUrl;
                }
                newChart.render();
                return;
            }
